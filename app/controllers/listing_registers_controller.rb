class ListingRegistersController < ApplicationController
  ...
  def index
    ...
    sql = "select * from (select @rowid:=@rowid+1 as rowid, id, ticker, name, home, btt, address, COALESCE(amount, 0) as amount from listing_requests, (select @rowid:=0) as init where listing_requests.aasm_state='approved' order by amount desc) as t where t.amount >= 1500";
