...
while($running) do
  ...
  begin
    ...
    for i in 1..7
      ...
      rvn  = aggregate(7, datefrom, dateto)
      da  = aggregate(8, datefrom, dateto)
      ac  = aggregate(9, datefrom, dateto)
      ep  = aggregate(10, datefrom, dateto)
      ad  = aggregate(11, datefrom, dateto)

      sql = "insert into turnover_summary (btc, ltc, doge, spero, mxt, rvn, da, ac, ep, ad, created_at, updated_at, slice) values (#{btc}, #{ltc}, #{doge}, #{spero}, #{mxt}, #{rvn}, #{da}, #{ac}, #{ep}, #{ad}, '#{datefrom}', null, #{ts})"
      res = ActiveRecord::Base.connection.execute(sql)
